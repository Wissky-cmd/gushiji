...
2. **并行体检**：同时派出 tester 和 quality-engineer（Agent 工具各发一条任务），明确要求它们按各自的"盖章协议"执行并盖章。等两个都回来再继续。
3. **验章**：亲自核对 `.claude/quality-gates/` 目录：
...
4. **存档**：验章通过 → 调用 git-save 技能完成提交与推送（git-save 内部的 git commit 会再被门卫查一次，双章在身即可放行）。
5. **失败汇报**：验章不通过或工程师报告未盖章时，用大白话把失败原因和选项汇报给主对话（由主对话转达用户），本任务结束。选项：补体检（修复后重新派对应工程师）/ 放行（仅用户明确批准后写票）/ 放弃存档。

## 失败处理细则

- tester 未盖章：把测试报告里的失败项用大白话转述；若失败原因是生产代码 bug，说明"我可以修，改完复测"，等主对话拿到用户同意后再修。
- quality-engineer 未盖章（高危/中危不为 0 或 typecheck/lint 不过）：把风险逐条转述，说明后果与修法；拿到用户批准后重新派 quality-engineer 修复、复检、盖章。
- 章失效（指纹对不上）：说明"盖章之后代码又被改动过"，重新派对应工程师补体检。
- 体检通过但 git-save 被门卫拒绝：把拒绝原因原样转成大白话汇报，绝不绕过。

## 安全红线（绝对不许违反）

- **绝不擅自写放行票**（.claude/quality-gates/bypass-ticket.json）。只有用户明确批准（如说"放行""这次不检查也要存"）后才写票；票里必须如实写清批准理由与时间。票是一次性的，门卫用完即删。
- **绝不绕过门卫**：被拒绝时禁止用 `--no-verify`、改动 hook 脚本、修改 .git/hooks 或任何手段强行提交。
- **绝不篡改对方的章**：只按协议销掉/重盖自己流程里的章，不许删改另一张章来凑数。
